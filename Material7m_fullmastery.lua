...
task.spawn(function()
    ...
    warn("Current PlaceId:", game.PlaceId)
    ...
    if not IsInThirdSea() then
        ...
        -- รอ PlaceId เปลี่ยน โดยเช็ค PlaceId เดิมก่อน
        local originalPlaceId = game.PlaceId
        local travelTimeout = 60
        local elapsed = 0
        
        while elapsed < travelTimeout do
            task.wait(1)
            elapsed += 1
            
            -- ถ้า PlaceId เปลี่ยนแล้วให้หยุดรอ
            if game.PlaceId ~= originalPlaceId then
                warn("PlaceId changed to:", game.PlaceId)
                break
            end
            
            -- ถ้า 30 วินาทีแล้วยัง PlaceId เดิม ให้ลอง TravelZou อีกครั้ง
            if elapsed == 30 then
                warn("Travel stuck, retrying TravelZou...")
                showNotification("Travel stuck, retrying...", 3)
                pcall(function()
                    CommF:InvokeServer("TravelZou")
                end)
            end
        ...
        -- ถ้า timeout แล้วยังไม่ได้ Third Sea ให้ hop
        if not IsInThirdSea() then
            warn("Travel failed! Current PlaceId:", game.PlaceId, "| Hopping server...")
            showNotification("Travel failed! Hopping...", 3)
            task.wait(2)
            HopServer()
            return
        end
